  ...
  try {
    ...
    const [gigs, total] = await Promise.all([
      Gig.find(filters)
        ...
        .skip(skip)
        .limit(safeLimit)
        .populate('teacher_id', 'name email roles rating description education certification experience isOnline profilePicture')
        .lean(),
...
const getGigById = async (req, res) => {
  try {
    ...
    const gig = await Gig.findById(id).populate(
      'teacher_id',
      'name email roles rating description education certification experience isOnline profilePicture'
    );
...
const getRecommendedGigs = async (req, res) => {
  try {
    ...
    const candidateGigs = await Gig.find(matchFilter)
      ...
      .limit(60)
      .populate('teacher_id', 'name email rating description education certification experience isOnline profilePicture')
      .lean();
